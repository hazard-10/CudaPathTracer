...
// return the number of paths that are still active for testing purposes
__global__ void countActivePathsegments(int nPaths, PathSegment *paths, int *out) {
    int index = (blockIdx.x * blockDim.x) + threadIdx.x;
